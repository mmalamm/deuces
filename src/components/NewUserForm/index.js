...
class NewUserForm extends Component {
  ...
  handleInputChange = e => {
    ...
    if (!usernameKey.match(/^[a-z0-9]{0,20}$/)) 
      return;
    const len = usernameKey.length;
    const buttonDisabled = len < 6;
    const buttonText = len < 6
      ? "6+ characters"
      : "submit";
  ...
  handleSubmit = async e => {
    ...
    if (buttonDisabled) 
      return;
    this.setState({buttonText: (<RotateLoading
      duration={1}
      stroke="#708090"
      opacity={0.4}
      size={30}
      className="NewUserForm-spinner"/>), buttonDisabled: true, inputDisabled: true});
    const usernameKey = username.toLowerCase();
  ...
  render() {
    ...
    const {username, inputDisabled, buttonDisabled, buttonText, showModal} = this.state;
    return (
      <div className="NewUserForm">
        ...
        <div
          className={`NewUserForm-button ${buttonDisabled
          ? "NewUserForm-buttonDisabled"
          : ""}`}
